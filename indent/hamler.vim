" indentation for hamler
"
" Based on idris indentation
"
" author: raichoo (raichoo@googlemail.com)
"
" Modify g:hamler_indent_if and g:hamler_indent_case to
" change indentation for `if'(default 3) and `case'(default 5).
" Example (in .vimrc):
" > let g:hamler_indent_if = 2
...
if !exists('g:hamler_indent_if')
  ...
  " >>>then ...
  " >>>else ...
  let g:hamler_indent_if = 3
endif

if !exists('g:hamler_indent_case')
  " case xs of
  " >>>>>[]     -> ...
  " >>>>>(y:ys) -> ...
  let g:hamler_indent_case = 5
endif

if !exists('g:hamler_indent_let')
  " let x = 0 in
  " >>>>x
  let g:hamler_indent_let = 4
endif

if !exists('g:hamler_indent_in')
  " let x = 0
  " >in
  let g:hamler_indent_in = 1
endif

if !exists('g:hamler_indent_where')
  " where
  " >>f :: Int -> Int
  " >>f x = x
  let g:hamler_indent_where = 6
endif

if !exists('g:hamler_indent_do')
  " do x <- a
  " >>>y <- b
  let g:hamler_indent_do = 3
endif

if !exists('g:hamler_indent_dot')
  " f
  "   :: forall a
  "   >. String
  "   -> String
  let g:hamler_indent_dot = 1
endif

setlocal indentexpr=GetHamlerIndent()
...
function! GetHamlerIndent()
  ...
  if line =~ '^\s*\<in\>'
    ...
    while s <= 0 && n > 0
      ...
      if s >= 0 && index(s:GetSynStack(v:lnum - 1, s), 'hamlerString') != -1
	let s = -1
      endif
    endwhile

    return s + g:hamler_indent_in
  endif
  ...
  if s >= 0 && prevline !~ '^class\>' && index(s:GetSynStack(v:lnum - 1, s), "hamlerFunctionDecl") == -1
  ...
  if s >= 0 && index(s:GetSynStack(s == 0 ? v:lnum - 1 : v:lnum, max([1, s])), "hamlerFunctionDecl") == -1
    \ && p >= 0 &&  index(s:GetSynStack(v:lnum - 1, p), "hamlerString") == -1
    return p
  endif
  ...
  if prevline =~ '^\s*\(::\|∷\)\s*forall'
    return match(prevline, '\S') + g:hamler_indent_dot
  endif
  ...
  if s >= 0 && index(s:GetSynStack(v:lnum - 1, s), 'hamlerString') == -1
    return s + g:hamler_indent_let
  endif

  let s = match(prevline, '\<let\>\s\+.\+\(\<in\>\)\?\s*$')
  if s >= 0 && index(s:GetSynStack(v:lnum - 1, s), 'hamlerString') == -1
    return match(prevline, '\<let\>') + g:hamler_indent_let
  endif
  ...
  if s >= 0 && index(s:GetSynStack(v:lnum - 1, s), 'hamlerString') == -1
    return match(prevline, '\S') + g:hamler_indent_where
  endif
  ...
  if s >= 0 && index(s:GetSynStack(v:lnum - 1, s), 'hamlerString') == -1
    return match(prevline, '\S') + g:hamler_indent_do
  endif
  ...
  if s >= 0 && index(s:GetSynStack(v:lnum - 1, s), 'hamlerString') == -1
    return match(prevline, '\<case\>') + g:hamler_indent_case
  endif
